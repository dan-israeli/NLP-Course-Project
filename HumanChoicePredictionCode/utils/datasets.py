...
class OnlineSimulationDataSet(Dataset):
    ...
    class SimulatedUser:
        def __init__(self, user_improve, basic_nature, favorite_topic_method, **args):
            ...
            self.ACTIONS = {
			    # the original stratigies

			    0: ("correct - oracle", 0, user_strategies.correct_action),
			    ...
			    # the newly defined basic language-based stratigies

			    6: ("LLM positive", basic_nature[5], user_strategies.LLM_positive), # Strictly Positive strategy
                            7: ("LLM negative", basic_nature[6], user_strategies.LLM_negative), # Strictly Negative strategy
                            8: ("LLM full", basic_nature[7], user_strategies.LLM_full) # Baseline strategy
                            }
